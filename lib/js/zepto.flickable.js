...
(function( $ ){
	...
	init : function( options ) {
		...
		var settings = $.extend( {
		  ...
		  snapSpeed		: 0.3, 
		  flickSnapSpeed: 0.3, 
		  moveThreshold : 0, //JX 20130724: touch move threshold value
				...
				var el 			= $(this),
					...
					if (!isAlive) { // Has the plugin already been initialised for this element?
						...
						var segments 		= settings.segments, 
						  ...
						  .data('moveThreshold', Math.abs(parseFloat(settings.moveThreshold))) //JX 20130724: touch move threshold value
						  .data('segment', 0)
	...
	move : function (callback) {
		...
		var el 		= $(this), pos, style, 
			...
			//JX 20130724: Added touchmove threshold to improve bi-axis touchmove UX
			dist	= eventData.delta.dist[d], 
			mvThresh= el.data('moveThreshold'), 
			pos		= Math.abs(dist) > mvThresh ? anchor + dist + (dist < 0 ? mvThresh : mvThresh * -1) : anchor + 0;
