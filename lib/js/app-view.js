...
window.App = (function(app){
	...
	var App = app || {},
		Scroller, articleScroller, weeksScroller;
	...
	App.view = {
		...
		initArticleScroller: function( $article ){
			...
			//destroy scroller instance
			if (articleScroller) articleScroller.destroy();
			//init scrolling plugin
			if (typeof(IScroll) == 'function') {
				articleScroller = new IScroll($container[0], {
					scrollbars: 			true
				});
			}
		},
		...
		onSegmentdidchange: function(e){
			...
			var Scroller = App.view.weeksScroller,
				$btnTabs = $('#nav').find('.navItem'),
				currentSeg = Scroller.currentSegment.x,
		...
		initArticleNavigator: function( $page ){			
			...
			var $articleNav = $('#articleNav'),
				$btnNavs = $articleNav.find('.navItem');				
		...
		initPage: function( $page ){
			...
			//common
			if ( hasScroller ){ App.view.initScroller(); console.log('has scroller');}
			...
			//weeks
				...
				(function(){
					...
					var Scroller = App.view.weeksScroller,
						...
						params = App.utils.getVarsFromSearch();
					//Scroller events
					Scroller.addEventListener('segmentdidchange', App.view.onSegmentdidchange);
