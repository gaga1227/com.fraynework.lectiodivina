...
window.App = (function(app){
	...
	App.view = {
		...
		initArticleAudio: function($article){
			...
			//update UI status
			this.updateBtnAudioStatus( hasAudio ? 'stopped' : 'disabled');
			...
			function onBtnAudio(e){
				...
				if (action == 'running') {
					//load audio
					App.utils.loadMedia(url, true);
					//update UI status
					this.updateBtnAudioStatus('running');
				} else {
					if (this.AudioPlayer) {
						this.AudioPlayer.stop();
						//this.AudioPlayer.release();
					...
					//update UI status
					this.updateBtnAudioStatus('stopped');
				}
		...
		//updateMediaStatus
		updateMediaStatus: function(status){
			//exit
			if (status < 0) return 'status code invalid';
			//update
			this.mediaStatus.status = status;
