...
window.App = (function(app){
	...
	App.utils = {
		...
		initBtnScroll: function(cls) {
			...
			$(btnScrollsCls).live('click', function(e){
				e.preventDefault();
				//vars
				var $btn = $(this),
					$target = $($btn.attr('data-target')),
					offset = parseInt($btn.attr('data-offset'), 10),
					scroller = $(scrollerCls)[0],
					currentPos = scroller.scrollTop,
					tgtPos =  $target.length ? ($target.offset().top + offset) : defaultPos;
				//anim step function
				function anim() {
					if (currentPos > tgtPos) {
						currentPos -= step;
						if ((currentPos + step) <= tgtPos) currentPos = tgtPos;
					} else if (currentPos < tgtPos) {
						currentPos += step;
						if ((currentPos + step) >= tgtPos) currentPos = tgtPos;
					}
					//console.log('Current ScrollTop: ', currentPos);
					scroller.scrollTop = currentPos;
					if (currentPos != tgtPos) {
						requestAnimationFrame(anim); //recursive call
					}
				}
				//use raf for non android due to scrollTop bug
				if ( !$.os.android ) { rafID = requestAnimationFrame(anim); }
			});
		},
